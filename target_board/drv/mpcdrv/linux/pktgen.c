				   ...
				   size_t count, loff_t * offset)
{
	...
	if (!strcmp(name, "add_device")) {
		...
		ret = count;
		printk(pg_result, "OK: add_device=%s", f);
		goto out;
	}
	...
	if (!strcmp(name, "rem_device_all")) {
		...
		ret = count;
		printk(pg_result, "OK: rem_device_all");
		goto out;
	}

	if (!strcmp(name, "max_before_softirq")) {
		printk(pg_result, "OK: Note! max_before_softirq is obsoleted -- Do not use");
		ret = count;
		goto out;
	}
...
static void pktgen_mark_device(const char *ifname)
{
	...
	mutex_lock(&pktgen_thread_lock);
	printk("pktgen: pktgen_mark_device marking %s for removal\n", ifname);
	...
	while (1) {
		...
		mutex_unlock(&pktgen_thread_lock);
		printk("pktgen: pktgen_mark_device waiting for %s "
				"to disappear....\n", ifname);
...
static void pktgen_stop_all_threads_ifs(void)
{
	...
	printk("pktgen: entering pktgen_stop_all_threads_ifs.\n");
...
static void pktgen_reset_all_threads(void)
{
	...
	printk("pktgen: entering pktgen_reset_all_threads.\n");
...
static void pktgen_rem_one_if(struct pktgen_thread *t)
{
	...
	printk("pktgen: entering pktgen_rem_one_if\n");
...
static void pktgen_rem_all_ifs(struct pktgen_thread *t)
{
	...
	printk("pktgen: entering pktgen_rem_all_ifs\n");
	if_lock(t);
...
static int pktgen_thread_worker(void *arg)
{
	...
	while (!kthread_should_stop()) 
	{
		...
		if (unlikely(!pkt_dev && t->control == 0)) {
			wait_event_interruptible_timeout(t->queue,t->control != 0,HZ/10);
			try_to_freeze();
			continue;
		}
	...
	printk("pktgen: %s stopping all device\n", t->tsk->comm);
	pktgen_stop(t);

	printk("pktgen: %s removing all device\n", t->tsk->comm);
	pktgen_rem_all_ifs(t);

	printk("pktgen: %s removing thread.\n", t->tsk->comm);
	pktgen_rem_thread(t);
...
static struct pktgen_dev *pktgen_find_dev(struct pktgen_thread *t,const char *ifname, bool exact)
{
	...
	if_unlock(t);
	printk("pktgen: find_dev(%s) returning %p\n", ifname, pkt_dev);
	return pkt_dev;
}
...
static int __init pktgen_create_thread(int cpu)
{
    ...
    printk("+++__init pktgen_create_thread+++\n\r");
	...
	pe = proc_create_data(t->tsk->comm, 0600, pg_proc_dir,&pktgen_thread_fops, t);
	if (!pe) {
		printk(KERN_ERR "pktgen: cannot create %s/%s procfs entry.\n",
				...
				struct pktgen_dev *pkt_dev)
{
	...
	printk("pktgen: remove_device pkt_dev=%p\n", pkt_dev);
