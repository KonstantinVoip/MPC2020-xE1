...
#
# DESCRIPTION
#   This test Local_bus Module for src file
#    
# REFERENCEs
# > References to software requirements specifications, hardware datasheets and other material
#
# NOTEs
# > Notes essential to this module (specific requirements, implementation restrictions and others)
***********************************************************************************************************************
#
# VERSION CONTROL
# > Keyword macros of file name, current revision and revision log
#
#   $Workfile: $
#
#   $Revision: $	
...
/* ##### Specific External Declarations ##### */
...
//static SINT32 nor_amdstd_read    (struct mtd_info *mtd, loff_t from, size_t len, size_t *retlen);
//static SINT32 nor_amdstd_read    (struct map_info *map, loff_t from, size_t len, size_t *retlen);  
//static SINT32 nor_do_read_onechip(struct map_info *map, struct flchip *chip, loff_t adr, size_t len);
...
#define DEBUG_ON       0  //0 - disable debugging , 1- enable debugging   
...
//SINT32 nor_do_read_onechip(struct map_info *map, struct flchip *chip, loff_t adr, size_t len)
{
...
// SINT32 nor_amdstd_read (struct map_info *map, loff_t from, size_t len, size_t *retlen)
//SINT32 nor_amdstd_read (struct mtd_info *mtd, loff_t from, size_t len, size_t *retlen)
...
/*SINT32  MPCInitNORflash(const struct of_device_id *match)*/
{
...
/**************************************************************************************************
Syntax:      	    static int __init mpc_localbusmodule_init(void)

Remarks:			This Function init point for mpc module.
...
Return Value:	Returns 1 on success and negative value on failure.
				...
				=-1												Failure
***************************************************************************************************/
static int __init mpc_localbusmodule_init(void)
{
 SINT32 status=1;	
 size_t len=0x00001000;
 loff_t from=0x0000000000000000;
 loff_t to  =0x0000000000ef4000;
 size_t w_len=0x00000040;
 size_t retlen;
 int i;
 //const u_char   buf[64];
 u_char  buf[0x1000];
 //
 printk(KERN_INFO "+++++++++++++++++++++++LocalBusModuleEnable+++++++++++++++++++++++++++:\n\r");	
  //
  //
  //p2020testsizeof();
  //p2020TestBus();
  // MPCInitNORflash(of_flash_match);
 MPCInitNORflash();      
 
 //SINT32 nor_amdstd_write_buffers(struct mtd_info *mtd, loff_t to, size_t len,size_t *retlen, const u_char *buf)
//   nor_amdstd_write_buffers(info->cmtd,to,w_len,&retlen,buf);
  //  nor_amdstd_read  (info->cmtd,from, len, &retlen);
      //nor_amdstd_read  (&info->list[0].map,from, len, &retlen);
   nor_amdstd_read  (map,from, len, &retlen,buf); 
   ...
   //info->list[0].map
#if 0
   status= MPCInitlocalBus();
   if(!status)
   {
	return STATUS_ERR;   
   }
   status=MPCDumplocalbusreg();
   if(!status)
   {	   
	return STATUS_ERR;
   }
...
/**************************************************************************************************
Syntax:      	    static void __exit mpc_localbusmodule_exit(void)

Remarks:			This Function exit point for mpc
...
Return Value:	Returns 1 on success and negative value on failure.
				...
				=-1												Failure
***************************************************************************************************/
static void __exit mpc_localbusmodule_exit(void)
{
	//kfree(mtd);
	kfree(cfi);
	kfree(pchip);
	kfree(map);
	//kfree(info);
	printk(KERN_INFO "-----------------------LocalBusModuleDisable-----------------------------:\n\r");
   
}


module_init(mpc_localbusmodule_init);
module_exit(mpc_localbusmodule_exit);

MODULE_LICENSE("GPL");
MODULE_AUTHOR("Konstantin Shiluaev <k_shilyaev@supertel.spb.su>");
MODULE_DESCRIPTION("Test LocalBusDriver driver module");
